...
generate_cron_from_env() {
    ...
    if [ -n "${CRON_CREATE_ROUND:-}" ] && [ -n "${CRON_DO_ROUND:-}" ] && [ -n "${CRON_CHANNEL_ID:-}" ]; then
        mkdir -p /logs
        {
            ...
            echo "${CRON_CREATE_ROUND} /usr/local/bin/python /app/manage.py create_round ${CRON_CHANNEL_ID}"
            echo "${CRON_DO_ROUND} /usr/local/bin/python /app/manage.py do_round_matching ${CRON_CHANNEL_ID}"
            echo ""
        } > /app/cron-jobs
    else
